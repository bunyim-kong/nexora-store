    ...
    <div class="flex items-center gap-4">
        <div class="toggle flex gap-6">
            <button id="dark-mode-toggle" class="dark-mode-toggle" aria-label="Toggle dark mode">
                ...
                <svg id="light-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-7">
            ...
            <a href="{{ route('cart.index') }}" class="relative">
                ...
                @else
                    <span class="cart-count absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 items-center justify-center" style="display: none;">
                        0
                    </span>
                @endif
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateIcons(theme) {
        if (theme === 'dark') {
            // In dark mode: show sun (light icon), hide moon (dark icon)
            darkIcon.style.display = 'none';
            lightIcon.style.display = 'block';
        } else {
            // In light mode: show moon (dark icon), hide sun (light icon)
